...
The installer reads the reth manifest, checks the backup height against live Telos mainnet, and refuses stale backups by default. The default staleness limit is `345600` blocks, about two days at 0.5 second block times.
...
To override the freshness guard intentionally:
```bash
ALLOW_STALE_BACKUPS=true bash run.sh --bootstrap-mode fast
```
...
- Resolves the latest dated nodeos snapshot instead of relying on an opaque `latest` alias
